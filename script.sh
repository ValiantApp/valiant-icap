...
APP_DIR="/opt/valiant-icap-server"
NODE_APP="index.js"
CONFIG_DIR="$APP_DIR"
PID_DIR="$APP_DIR/pid"
...
[ -f /etc/default/valiant-icap-server ] && . /etc/default/valiant-icap-server

if [ -z "$METACERT_KEY" ] ;  then
  echo "METACERT_KEY is not set, please set it in /etc/default/valiant-icap-server" >&2
  exit 1
fi
...
start_app() {
export METACERT_KEY=EmEyL8twWzOc4YaC
    if pid_file_exists
    then
        if is_running
        then
